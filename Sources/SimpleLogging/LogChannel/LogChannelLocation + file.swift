...
public class FileLogChannelLocation: LogChannelLocation {
    ...
    ///   - createIntermediariesIfNecessary: _optional_ - If the parent directory, or any of its ancestor directories, doesn't exist, then passing `true` will cause this to attempt to create it, whereas if you pass `false`, then this will throw an error if they don't exist. Defaults to `true`.
    ...
    public convenience init(filePath: String, createIntermediariesIfNecessary: Bool = true, using fileManager: FileManager = .default) throws {
        try self.init(fileAt: URL(fileURLWithPath: filePath), createIntermediariesIfNecessary: createIntermediariesIfNecessary, using: fileManager)
    }
    ...
    ///
    /// - Throws: If the parent directory cannot be created, or if you specified that it shouldn't be but it doesn't exist, or if the log file cannot ce created, or if the file couldn't be opened for writing
    public init(fileAt pathUrl: URL, createIntermediariesIfNecessary: Bool = true, using fileManager: FileManager = .default) throws {
        let parentDirectory = pathUrl.deletingLastPathComponent()
        
        do {
            try fileManager.createDirectory(at: parentDirectory, withIntermediateDirectories: createIntermediariesIfNecessary, attributes: nil)
        }
        catch {
            print(error)
...
public extension LogChannel where Location == FileLogChannelLocation {
    ...
    static func file(
        ...
        createIntermediariesIfNecessary: Bool = true,
    ...
    throws -> Self {
        LogChannel(
            ...
            location: try FileLogChannelLocation(
                filePath: filePath,
                createIntermediariesIfNecessary: createIntermediariesIfNecessary,
                using: fileManager),
